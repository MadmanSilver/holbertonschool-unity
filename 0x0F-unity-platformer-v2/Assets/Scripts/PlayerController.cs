...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!pm.paused) {
            ...
            if (canMove) {
                float horizontal = 0f;
                if (Input.GetKey(PlayerPrefs.GetString("Right").ToLower()))
                    horizontal += 1f;
                if (Input.GetKey(PlayerPrefs.GetString("Left").ToLower()))
                    horizontal -= 1f;
                    
                float vertical = 0f;
                if (Input.GetKey(PlayerPrefs.GetString("Forward").ToLower()))
                    vertical += 1f;
                if (Input.GetKey(PlayerPrefs.GetString("Backward").ToLower()))
                    vertical -= 1f;
                ...
                if (Input.GetKeyDown(PlayerPrefs.GetString("Jump").ToLower()) && isGrounded) {
                ...
                if (tutorial != null) {
                    ...
                    if (Mathf.Abs(Input.GetAxisRaw("LookVertical")) > 0.1f || Mathf.Abs(Input.GetAxisRaw("LookHorizontal")) > 0.1f) {
                        tutLook = true;
